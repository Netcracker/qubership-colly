...
| Environment  | `cmApproach`            |                  | enum [`CMDB`, `NO_CMDB`, `NO_CMDB_V2`]                                                                      | instance     | `inventory.deployer` sets base `CMDB`/`NO_CMDB`; `inventory.noCmdbVersion` (`v1`/`v2`) overrides it | CMDB, noCMDB, or noCMDB-v2 approach is used for environment |
...
inventory:
  ...
  # deployer: "https://deployer.example.com"  # if present -> base cmApproach = CMDB, otherwise NO_CMDB
  # noCmdbVersion: "v2"                       # overrides the base value regardless of deployer:
                                               #   "v2" -> cmApproach = NO_CMDB_V2
                                               #   "v1" -> cmApproach = NO_CMDB
                                               #   absent/other -> base value from deployer is kept
